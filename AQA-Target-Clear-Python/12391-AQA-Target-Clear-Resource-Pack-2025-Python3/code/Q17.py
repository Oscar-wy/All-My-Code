...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        #CHANGE
        UserChoice = input("Enter 1 if you would like to give your expression in Infix Notation and 2 to give it in RPN: ")
        RPNInput = False
        if UserChoice == "2":
            print()
            print("Enter your RPN using a COMMA between the operators and operands")
            print()
            RPNInput = True
        UserInput = input("Enter an expression: ")
        print()
        if CheckIfUserInputValid(UserInput, RPNInput):
            UserInputInRPN = ConvertToRPN(UserInput, RPNInput)
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
                    NumbersAllowed = RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed, RPNInput)
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        #END CHANGE
        Score -= 1
        if Targets[0] != -1:
...
#CHANGE  
def RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed, RPNInput):
    UserInputInRPN = ConvertToRPN(UserInput, RPNInput)
    #END CHANGE
...
#CHANGE
def ConvertToRPN(UserInput, RPNInput):
    if RPNInput:
...
#CHANGE
def CheckIfUserInputValid(UserInput, RPNInput):
    if RPNInput:
            return True
    elif re.search("^([0-9]+[\\+\\-\\*\\/])+[0-9]+$", UserInput) is not None:
        return True
    else:
        return False
#END CHANGE
