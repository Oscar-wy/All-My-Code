    ...
    if UserInputEvaluation != -1:
        for Count in range(0, len(Targets)):
            if Targets[Count] == UserInputEvaluation:
                ...
                TargetsFound += 1
    print(f"Your expression evaluated to {UserInputEvaluation} and was found {TargetsFound} times.")
    print()
    ...
    for Item in UserInputInRPN:
        if CheckValidNumber(Item, MaxNumber):
            if int(Item) in Temp:
                ...
                InvalidNumbers.append(int(Item))
                AllNumbersValid = False
    if not AllNumbersValid:
        print("Your expression is not valid because the following numbers are not available to you:")    
        for Item in InvalidNumbers:
          print(str(Item) + ",  ", end = '')
        print()
        print()        
