...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        #CHANGE
        if len(UndoTurns) > 0:
            UserChoice = input(f"There are {len(UndoTurns)} Undos left. Would you like to Undo the last turn? Y/N").upper()
            if UserChoice == "Y":
                UndoTurns, NumbersAllowed, Targets, Score = UndoLastTurn(UndoTurns, NumbersAllowed, Targets, Score)
                DisplayState(Targets, NumbersAllowed, Score)
        UndoTurns = AddToUndo(UndoTurns, NumbersAllowed, Targets, Score)
        #END CHANGE
...
#CHANGE
def UndoLastTurn(UndoTurns, NumbersAllowed, Targets, Score):
    Targets = []
    NumbersAllowed = []
    LastMove = UndoTurns.pop()
    Score = LastMove.GetScore()
    Targets = LastMove.GetTargets()
    NumbersAllowed = LastMove.GetNumbersAllowed()
    return UndoTurns,NumbersAllowed,Targets,Score

def AddToUndo(UndoTurns, NumbersAllowed, Targets, Score):
    LastTargetsState = []
    LastNumbersAllowedState = []
    for Item in Targets:
        LastTargetsState.append(Item)
    for Item in NumbersAllowed:
        LastNumbersAllowedState.append(Item)
    UndoTurns.append(UndoState(LastTargetsState,LastNumbersAllowedState,Score))
    return UndoTurns
#END CHANGE
...
class UndoState():
    def __init__(self, Targets, NumbersAllowed, Score):
        ...
        self.__NumbersAllowed = NumbersAllowed
        self.__Score = Score
