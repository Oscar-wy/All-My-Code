...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                #CHANGE
                TargetToAddToNumbersAllowed = -1
                TargetToAddToNumbersAllowed, IsTarget, Score = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score, TargetToAddToNumbersAllowed)
                if IsTarget:
    ...
    DisplayScore(Score)
#CHANGE
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score, TargetoAddToNumbersAllowed):
    UserInputEvaluation = EvaluateRPN(UserInputInRPN)
    UserInputEvaluationIsATarget = False
    if UserInputEvaluation != -1:
        for Count in range(0, len(Targets)):
            if Targets[Count] == UserInputEvaluation:
                SuccessfulTarget = Targets[Count]
                Score += 2
                Targets[Count] = -1
                UserInputEvaluationIsATarget = True
                print("Target Successfully hit")      
                UserChoice = input(f"Would you like to add either the successful target {SuccessfulTarget} or parts of it to the Allowed Numbers List? Y/N").upper()  
                if UserChoice == "Y":
                    TargetoAddToNumbersAllowed = SelectValueFromTarget(SuccessfulTarget)
    return TargetoAddToNumbersAllowed, UserInputEvaluationIsATarget, Score
    #END CHANGE

#CHANGE
def SelectValueFromTarget(SuccessfulTarget):
    FirstDigit = SuccessfulTarget // 10
    SecondDigit = SuccessfulTarget % 10
    print("Please select from the options below which part of the target or the target itself you would like to include in the Allowed Numbers:")
    print(f"1: The first digit: {FirstDigit}")
    print(f"2: The second digit: {SecondDigit}")
    print(f"3: The whole target: {SuccessfulTarget}")
...
#CHANGE    
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, TargetToAddToNumbersAllowed = -1):
    if TargetToAddToNumbersAllowed == -1:
        ...
        if TrainingGame:
            return [2, 3, 2, 8, 512, TargetToAddToNumbersAllowed]
        else:
            while len(NumbersAllowed) < 5:
                NumbersAllowed.append(GetNumber(MaxNumber))     
            NumbersAllowed.append(TargetToAddToNumbersAllowed) 
            return NumbersAllowed
#END CHANGE
