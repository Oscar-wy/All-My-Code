...
#CHANGE
def ConvertToRPNWithBrackets(UserInput, MaxNumber):
    Operators = []
    RPNQueue = []
    FullInfixExpression = re.split("(\D)", UserInput)
    CleanedInfixExpression = []
    for Item in FullInfixExpression:
        if Item is not None and Item != "":
            CleanedInfixExpression.append(Item)
    Precedence = {"(": 1, "+": 2, "-": 2, "*": 4, "/": 4}
    for Item in CleanedInfixExpression:
            ...
            if Item == "(":
                ...
                while len(Operators) > 0:
                    if Operators[-1] != "(":
                        ...
                        Operators.pop()
                        break
            elif len(Operators) > 0 and Precedence[Operators[-1]] > Precedence[Item]:
                RPNQueue.append(Operators.pop())
                Operators.append(Item)
            else:
                Operators.append(Item)
        elif CheckValidNumber(Item, MaxNumber):
            RPNQueue.append(Item) 
    while len(Operators) > 0:
        RPNQueue.append(Operators.pop())
    return RPNQueue
#END CHANGE
...
def CheckIfUserInputValid(UserInput):
    if re.search("^\(*([0-9]+|[\+\-\*\/\(\)])+\)*$", UserInput) is not None:
        return True
    else:
        return False
#END CHANGE
