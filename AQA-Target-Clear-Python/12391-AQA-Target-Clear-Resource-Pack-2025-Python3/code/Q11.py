...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        #CHANGE
        UserChoice = input("Would you like up to five random suggestions of solutions involving only 2 or 3 operands? Y/N").upper()
        print()
        if UserChoice == "Y":
...
#CHANGE
def GetRandomSuggestions(NumbersAllowed, Targets):
    Suggestions = set()
    SolutionsTested = 0
    while (len(Suggestions) < 5):
        ...
        for Item in NumbersAllowed:
            Temp.append(Item)
        FirstOperand = Temp[random.randint(0,len(Temp)-1)]
        Temp.remove(FirstOperand)
        SecondOperand = Temp[random.randint(0,len(Temp)-1)]
        Temp.remove(SecondOperand)
        Operator = random.randint(0,3)
        Result = 0
        if Operator == 0:
            Result = FirstOperand + SecondOperand
            if (Result in Targets and Result != -1):
            ...
            if float(FirstOperand / SecondOperand) - math.floor(FirstOperand / SecondOperand) == 0.0:
        ...
        ThirdOperand = Temp[random.randint(0,len(Temp)-1)]
        Operator = random.randint(0,3)
        if Operator == 0:
            ...
            if float(Result / ThirdOperand) - math.floor(Result / ThirdOperand) == 0.0:
                Result = math.floor(Result / ThirdOperand)
            if (Result in Targets and Result != -1):
                Suggestions.add(Result)      
        elif Operator == 3:
            Result = Result * ThirdOperand
...
#CHANGE
def DisplayState(Targets, NumbersAllowed, Score, Suggestions = NULL):
    if Suggestions != NULL:
        DisplayTargets(Targets, Suggestions)
    else:
...
def DisplayTargets(Targets, Suggestions = NULL):
    print("|", end = '')
    for T in Targets:
        if T == -1:
            print(" ", end = '')
