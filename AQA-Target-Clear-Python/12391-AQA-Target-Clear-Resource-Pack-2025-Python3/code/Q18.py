...
def CreateTargets(SizeOfTargets, MaxTarget):
    ...
    #CHANGE
    UniqueNumbers = set()
    while len(UniqueNumbers) < SizeOfTargets - 5:
        UniqueNumbers.add(GetTarget(MaxTarget))
    Targets.extend(UniqueNumbers)
    #END CHANGE
    return Targets
...
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber):
    if TrainingGame:
        ...
        while len(UniqueNumbers) < 5:
            UniqueNumbers.add(GetNumber(MaxNumber))    
        return list(UniqueNumbers)
        #END CHANGE
