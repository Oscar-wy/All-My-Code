...
def GenerateEvaluations(NumbersAllowed, Targets):
    PossibleExpressions = {}
    GenerateEvaluationsHelper(NumbersAllowed, Targets, 0, NumbersAllowed[0], PossibleExpressions, str(NumbersAllowed[0]))
    return PossibleExpressions

def GenerateEvaluationsHelper(NumbersAllowed, Targets, Index, CurrentResult, PossibleExpressions, CurrentExpression):
    if Index == len(NumbersAllowed)-1:
        ...
        if EvaluateRPN(ConvertToRPN(CurrentExpression)) in Targets and EvaluateRPN(ConvertToRPN(CurrentExpression)) != -1:
            if EvaluateRPN(ConvertToRPN(CurrentExpression)) not in PossibleExpressions:
                PossibleExpressions[EvaluateRPN(ConvertToRPN(CurrentExpression))] = CurrentExpression
        return
    NextNumber = NumbersAllowed[Index + 1]
    GenerateEvaluationsHelper(NumbersAllowed, Targets, Index + 1, CurrentResult * NextNumber, PossibleExpressions, f"{CurrentExpression}*{NextNumber}")
    if NextNumber != 0:
        if float(CurrentResult / NextNumber) - math.floor(float(CurrentResult / NextNumber)) == 0.0:
            GenerateEvaluationsHelper(NumbersAllowed, Targets, Index + 1, math.floor(CurrentResult / NextNumber), PossibleExpressions, f"{CurrentExpression}/{NextNumber}")
    GenerateEvaluationsHelper(NumbersAllowed, Targets, Index + 1, CurrentResult + NextNumber, PossibleExpressions, f"{CurrentExpression}+{NextNumber}")
    GenerateEvaluationsHelper(NumbersAllowed, Targets, Index + 1, CurrentResult - NextNumber, PossibleExpressions, f"{CurrentExpression}-{NextNumber}")
#END CHANGE
