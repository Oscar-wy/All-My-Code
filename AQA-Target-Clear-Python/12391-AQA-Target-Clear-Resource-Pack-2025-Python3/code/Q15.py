...
##### Test Encoded game state to test with: @@@@@Qf_Hg@kCCN@WJhSCHGJAC
def Main():                     #Q15
    NumbersAllowed = []
    Targets = []
    ...
    #CHANGE
    Score = 0
    Choice = input("Enter y to play the training game, L to restore a previous game state, anything else to play a random game: ").upper()
    print()
    if Choice == "Y":
        MaxNumber = 1000
        MaxTarget = 1000
        ...
        Targets, NumbersAllowed, Score = RestoreGame(Targets, NumbersAllowed, Score)
        TrainingGame = False
        MaxNumber = 10
        MaxTarget = 50    
    else:
...
#CHANGE
def RestoreGame(Targets, NumbersAllowed, Score):
    RestoreCode = ""
    ValidCode = False
    while (not ValidCode):
        RestoreCode = input("Enter in the game restore code. It should be 26 characters.")
        if re.search("^[@A-z[\\]^_']+$", RestoreCode) is not None:
            if len(RestoreCode) == 26:
                if "@" in RestoreCode[20:]:
                    print("That is not a valid code")
                else:
                    ValidCode = True
            else:
                print("That code is not the right length")
        else:
            print("That code contains invalid characters")
    for i in range(20):
        if ord(RestoreCode[i]) - 64 == 0:
            Targets.append(-1)
        else:
            Targets.append(ord(RestoreCode[i]) - 64)
    for i in range(20,25):
        NumbersAllowed.append(ord(RestoreCode[i]) - 64)
    Score = ord(RestoreCode[25]) - 64
    print()
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, Score):
    #END CHANGE (removed Score from scope and passing in as a parameter)
    GameOver = False
    while not GameOver:
