...
def Main():
    ...
    if Choice == "y":
        ...
        print("Please select from one of the below game options:")
        print("1: Standard Game")
        print("2: Easy Game - One large number and 4 small numbers")
        print("3: Medium Game - Two large numbers and 3 small numbers")
        print("4: Hard game - Four large numbers and 1 small number")
        GameChoiceType = int(input())
        MaxNumber = 10
        MaxTarget = 50
...
#CHANGE    
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, GameChoiceType):
    Score = 0
    GameOver = False
    while not GameOver:
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, GameChoiceType)   ##CHANGED HERE
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
        else:
...
def CheckValidNumber(Item, MaxNumber):
    if re.search("^[0-9]+$", Item) is not None:
        ...
        if ItemAsInteger > 0 and ItemAsInteger <= MaxNumber or ItemAsInteger in Temp:
            return True            
        #END CHANGE
    return False
...
#CHANGE    
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, GameChoiceType):
    LargeNumbers = [25, 50, 75, 100]
    if TrainingGame:
        return [2, 3, 2, 8, 512]
    elif GameChoiceType == 2:
        NumbersAllowed = []
        NumbersAllowed.append(LargeNumbers[random.randint(0,3)])
        for i in range(4):
            NumbersAllowed.append(GetNumber(MaxNumber))
    elif GameChoiceType == 3:
        NumbersAllowed = []
        NumbersAllowed.append(LargeNumbers[random.randint(0,3)])
        NumbersAllowed.append(LargeNumbers[random.randint(0,3)])
        for i in range(3):
            NumbersAllowed.append(GetNumber(MaxNumber))
    elif GameChoiceType == 4:
        NumbersAllowed = []
        for i in range(4):
            NumbersAllowed.append(LargeNumbers[random.randint(0,3)])
        NumbersAllowed.append(GetNumber(MaxNumber))
