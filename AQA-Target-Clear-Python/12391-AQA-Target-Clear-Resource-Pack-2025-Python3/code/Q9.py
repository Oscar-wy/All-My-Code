...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    GameOver = False
    while not GameOver:
        DisplayState(Targets, NumbersAllowed, Score, FrozenPosition)
        if FrozenPosition == -1:
            UserChoice = input("Would you like to freeze the position of one of the targets? Y/N ").upper()
            if UserChoice == "Y":
                FrozenPosition = int(input("Please enter in a position to freeze between 1 and 20:")) - 1
                DisplayState(Targets, NumbersAllowed, Score, FrozenPosition)
        else:
            UserChoice = input(f"Would you like to unfreeze the position of target {FrozenPosition + 1}? Y/N").upper()
            if UserChoice == "Y":
                FrozenPosition = -1
                DisplayState(Targets, NumbersAllowed, Score, FrozenPosition)
        ...
        if Targets[0] != -1:
            ...
            Targets = UpdateTargets(Targets, TrainingGame, MaxTarget, FrozenPosition)        
    print("Game over!")
    DisplayScore(Score)
    #END CHANGE
...
#CHANGE
def UpdateTargets(Targets, TrainingGame, MaxTarget, FrozenPosition):
    for Count in range (0, len(Targets) - 1):
        if Count == FrozenPosition:
...
#CHANGE
def DisplayState(Targets, NumbersAllowed, Score, FrozenPosition):
    DisplayTargets(Targets, FrozenPosition)
...
#CHANGE    
def DisplayTargets(Targets, FrozenPosition):
    Counter = 0
    print("|", end = '')
    for T in Targets:
        if T == -1:
